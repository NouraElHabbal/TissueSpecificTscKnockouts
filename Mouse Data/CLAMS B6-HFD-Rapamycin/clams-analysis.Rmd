...
merged_data <- 
  ...
  mutate(Total.Weight = Lean + Fat,
         ...
         Heat = Heat.bw * Total.Weight/1000*1000,
...
plot.data <- grouped.summary %>% filter(HFD==T) %>% select(Treatment, Light.Dark, Heat) %>% spread(Light.Dark,Heat)
plot.data.se <- grouped.summary.se %>% filter(HFD==T) %>% select(Treatment, Light.Dark, Heat) %>% spread(Light.Dark,Heat)
