...
library(rjson)
library(RCurl)
#download data from experiment 796
itt_url <- 'http://bridgeslab.uthsc.edu/mousedb/api/v1/data/?format=json&username=davebridges&api_key=fce3fde2a9a2e5dc9e04c20aad90120a621c50b3&limit=0&experiment=796'
...
itt.glucose.no.outlier <- subset(itt.glucose, id !=36958)
```

```{r ptt-analysis, dev=c('png','pdf')}
...
plot <- plot(times,
     ...
     las=1, ylim=c(0,ymax), ylab="Blood Glucose (mg/dL)", xlab="Pyruvate (min)", type="l", col='black')
#add other lines
lines(times, as.numeric(itt.summary.means[2,3:dim(itt.summary.se)[2]]), col='red')
lines(times, as.numeric(itt.summary.means[3,3:dim(itt.summary.se)[2]]), col='black', lty=2)
lines(times, as.numeric(itt.summary.means[4,3:dim(itt.summary.se)[2]]), col='red', lty=2)
#add error bars
...
superpose.eb(times,
             ...
             as.numeric(itt.summary.se[3,3:dim(itt.summary.means)[2]]), col='black')
#add error bars for dex
superpose.eb(times,
             as.numeric(itt.summary.means[4,3:dim(itt.summary.means)[2]]),
             as.numeric(itt.summary.se[4,3:dim(itt.summary.means)[2]]), col='red')
#add legend
legend("topright",
       paste(itt.summary.means$Treatment, itt.summary.means$Diet,sep=" - "),
       col=c('red','black','red','black'), lty=c(1,1,2,2), bty='n')
...
ymax <- max(itt.summary.means.norm[,3:dim(itt.summary.means.norm)[2]],na.rm=T) + max(itt.summary.se.norm[,3:dim(itt.summary.se.norm)[2]],na.rm=T)
plot <- plot(times,
     as.numeric(itt.summary.means.norm[1,1:dim(itt.summary.means.norm)[2]]),
     las=1, ylim=c(0,ymax), ylab="Blood Glucose (% initial)", xlab="Pyruvate (min)", type="l", col='black')
#add other lines
lines(times, as.numeric(itt.summary.means.norm[2,1:dim(itt.summary.se.norm)[2]]), col='red')
lines(times, as.numeric(itt.summary.means.norm[3,1:dim(itt.summary.se.norm)[2]]), col='black', lty=2)
lines(times, as.numeric(itt.summary.means.norm[4,1:dim(itt.summary.se.norm)[2]]), col='red', lty=2)
...
superpose.eb(times,
             ...
             as.numeric(itt.summary.se.norm[3,1:dim(itt.summary.means.norm)[2]]), col='black')
#add error bars for dex
superpose.eb(times,
             as.numeric(itt.summary.means.norm[4,1:dim(itt.summary.means.norm)[2]]),
             as.numeric(itt.summary.se.norm[4,1:dim(itt.summary.means.norm)[2]]), col='red')
# #add legend
# legend("topright",
#        paste(itt.summary.means$Treatment, itt.summary.means$Diet,sep=" - "),
#        col=c('red','black','red','black'), lty=c(1,1,2,2), bty='n')
...
```{r ptt-analysis-no-outlier, dev=c('png','pdf')}
#generate averages/se/n/shapiro test for each time point
se <- function(x) sd(x, na.rm=T)/sqrt(length(x))
itt.summary.means <- 
  itt.glucose.no.outlier %>%
  group_by(Treatment, Diet) %>%
  summarize(
    t0 = mean(t0, na.rm=T),
    t15 = mean(t15, na.rm=T),
