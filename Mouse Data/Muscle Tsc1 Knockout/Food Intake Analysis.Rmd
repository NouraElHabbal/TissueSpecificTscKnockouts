...
ggplot(cage.summary.data,
  ...
  facet_grid(~Sex)+ labs(x="Diet",y="average kcals consumed per mouse per day")+
  ggtitle("Average Caloric Intake")
...
anova(intake.null, intake.lme.hfd)#Diet assignment will be included in final model

intake.lme.age <- lmer(calorie.intake ~ HFD + age + (1|Cage), data=exp.data.22)
anova(intake.lme.hfd, intake.lme.age)#age will be inlcuded in final model

intake.lme.sex <- lmer(calorie.intake ~ HFD + age + Sex + (1|Cage), data=exp.data.22) 
anova(intake.lme.sex, intake.lme.age)#Although sex isn't significant, it will be included in the final model, as it was part of the research question

library(broom)
#final model
intake.lme.geno <- lmer(calorie.intake ~ HFD + age + Sex + Genotype + (1|Cage), data=exp.data.22) 
anova(intake.lme.sex, intake.lme.geno) %>% kable

intake.lme.geno %>% anova %>% kable #Statistical significance of the final model

intake.lme.geno %>% fixef %>% kable #generates the effect estimates of the full model
