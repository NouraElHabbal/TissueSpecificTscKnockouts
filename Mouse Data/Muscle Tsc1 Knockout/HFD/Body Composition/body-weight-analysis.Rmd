...
Based on this formula the knockout caused a `r wt.predict.m-ko.predict.m`g increase in body weight for males and a `r wt.predict.f-ko.predict.f`g increase for females (p=`r anova(weight.lme, weight.lme.null)$"Pr(>Chisq)"[2]` via a Chi-squared test).
...
#statistics
lean.lme <- lmer(Lean.Mass~age*Gender*Knockout + (1|animal.id) + (age-1|animal.id), data=subset(weight.data,age<12*7+70), REML=F)
lean.lme.null <- lmer(Lean.Mass~age*Gender + (1|animal.id) + (age-1|animal.id), data=subset(weight.data,age<12*7+70), REML=F)

wt.predict.m <- predict(lean.lme, newdata=list(age=7*12+70, Gender="M", Knockout=FALSE, animal.id=weight.data$animal.id[1]))
ko.predict.m <- predict(lean.lme, newdata=list(age=7*12+70, Gender="M", Knockout=TRUE, animal.id=weight.data$animal.id[1]))
...
Lean Mass ~ `r as.character(formula(lean.lme))[3]`

Based on this formula the knockout caused a `r wt.predict.m-ko.predict.m`g increase in lean mass for males and a `r wt.predict.f-ko.predict.f`g increase for females (p=`r anova(lean.lme, lean.lme.null)$"Pr(>Chisq)"[2]` via a Chi-squared test).
...
#statistics
fat.lme <- lmer(Total.Fat.Mass~age*Gender*Knockout + (1|animal.id) + (age-1|animal.id), data=subset(weight.data,age<12*7+70), REML=F)
fat.lme.null <- lmer(Total.Fat.Mass~age*Gender + (1|animal.id) + (age-1|animal.id), data=subset(weight.data,age<12*7+70), REML=F)

wt.predict.m <- predict(fat.lme, newdata=list(age=7*12+70, Gender="M", Knockout=FALSE, animal.id=weight.data$animal.id[1]))
ko.predict.m <- predict(fat.lme, newdata=list(age=7*12+70, Gender="M", Knockout=TRUE, animal.id=weight.data$animal.id[1]))
...
Fat Mass ~ Weight ~ `r as.character(formula(fat.lme))[3]`

Based on this formula the knockout caused a `r wt.predict.m-ko.predict.m`g increase in fat mass for males and a `r wt.predict.f-ko.predict.f`g increase for females (p=`r anova(fat.lme, fat.lme.null)$"Pr(>Chisq)"[2]` via a Chi-squared test).
...
Based on this formula the knockout caused a `r wt.predict.m-ko.predict.m` increase in percent fat mass for males and a `r wt.predict.f-ko.predict.f` increaes for females (p=`r anova(pct.fat.lme, pct.fat.lme.null)$"Pr(>Chisq)"[2]` via a Chi-squared test).
...
summary.census <-
   ...
   distinct(animal.id) %>%
   summarize(Number = length(age))
kable(summary.census, caption="Animals which have completed the study")
