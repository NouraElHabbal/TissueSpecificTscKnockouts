...
baseline.data <-  
  ...
  bind_rows() %>%
  select(Subject, Interval, `Date/Time`, `Light/Dark`, `Volume O2`, RER, `X Ambulatory`, `Y Ambulatory`,`Volume CO2`) %>%
  ...
  mutate(Heat = (3.815 + 1.232*RER)*`Volume O2`*1.163/1000) %>% #from Lusk Equation, and converted to kW
...
ggplot(summary.data,
  ...
  geom_errorbar() +
    facet_grid(~Sex) +
  labs(y="Volume O2 Consumed (mL/h)", x="Zeitgerber Time") +
...
ggplot(summary.data,
  ...
  geom_line() +
  geom_errorbar() +
  labs(y="Heat Production (kW)", x="Zeitgerber Time") 
...
animal.data.ld.summary %>% 
  ...
  kable(caption="Average changes in heat production form wt to knockout")
```
