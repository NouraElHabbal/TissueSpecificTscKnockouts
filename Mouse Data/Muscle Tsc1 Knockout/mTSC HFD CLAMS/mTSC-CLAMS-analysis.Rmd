...
animal.data.ld <-
  ...
  mutate(CHO.Ox = (4.585 * `Volume CO2`*60/1000) - (3.226 * `Volume O2`*60/1000)) %>% #converted gas rates from mL/h to L/min to get this in g/min
  mutate(Lipid.Ox = (1.695 * `Volume O2`*60/1000) - (1.701 * `Volume CO2`*60/1000)) %>% #converted gas rates from mL/h to L/min to get this in g/min
...
ggplot(summary.data,
  ...
  geom_line() +
  geom_errorbar() +
  labs(y='Carbohydrate Oxidation (g/min)', x="Zeitgeber Time",) +
  facet_grid(~Sex) +
  theme_classic() +
  scale_color_manual(labels = c("Wild-Type", "Knockout"), values=c("#6CBE45","#FAC917")) +
  theme(legend.position = c(0.15, 0.9),
...
ggplot(summary.data,
  ...
  geom_line() +
  geom_errorbar() +
  labs(y="Lipid Oxidation (g/min)", x="Zeitgeber Time") +
  facet_grid(~Sex) +
  theme_classic() +
  scale_color_manual(labels = c("Wild-Type", "Knockout"), values=c("#6CBE45","#FAC917")) +
  theme(legend.position = c(0.15, 0.9),
...
ggplot(animal.data.ld %>% filter(Knockout=="Control"),
  ...
  labs(title="Relationship between fat and lipid oxidation",
       y="Carbohydrate oxidation (g/min)",
       x="Lipid oxidation (g/min)")
