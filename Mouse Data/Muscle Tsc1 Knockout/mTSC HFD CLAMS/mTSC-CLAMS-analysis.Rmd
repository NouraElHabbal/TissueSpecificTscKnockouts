...
animal.data <-
  ...
  mutate(CHO.Ox = (4.585 * `Volume CO2`) - (3.226 * `Volume O2`)) %>% # to get this in g/min
  summarize(RER = mean(RER,na.rm=T),
            Activity = mean(Activity, na.rm=T),
...
```{r cho-analysis-linegraph, fig,cap="Mouse level RER levels"}
ggplot(animal.data,
...
```{r cho-summarized-data, fig.cap="Linegraph of RER Data"}
